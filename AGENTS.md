...
- Buff/Debuff development should use `src/data/statusEffectTaxonomy.json` and `docs/status-effect-taxonomy.md` as the management table. Status effects should carry a `stackingKey` and `stackingSide`; same key + same side overwrite, while similar effects from different sides can coexist.
